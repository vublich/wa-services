...
// ─── Logger — warn level so we see Baileys internal errors ───────────────────
...
async function handleMessage(sock, msg) {
    ...
    // Log EVERYTHING so we can see what arrives (remove after debugging)
    console.log(`[msg] jid=${remoteJid} fromMe=${msg.key.fromMe} keys=${Object.keys(msg.message).join(",")}`);

    // View-once check — must be BEFORE fromMe filter
...
async function startBot() {
    ...
    const sock = makeWASocket({
        ...
        // receive all pending messages on reconnect
    ...
    // Accept ALL upsert types — not just "notify" — so we catch everything
